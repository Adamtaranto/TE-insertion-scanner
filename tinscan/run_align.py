...
def mainArgs():
	parser = argparse.ArgumentParser(description='Align B genome (query) sequences onto A genome (target) using LASTZ.',prog='tinscan-align')
	# Input options
	parser.add_argument('--adir',type=str,required=True,help='Name of directory containing sequences from A genome.')
	parser.add_argument('--bdir',type=str,required=True,help='Name of directory containing sequences from B genome.')
	parser.add_argument('--pairs',type=str,default=None,help='Optional: Tab-delimited 2-col file specifying target:query sequence pairs to be aligned')
	# Output options
	...
	parser.add_argument('--hspthresh',type=int,default=3000,help='LASTZ min HSP threshold. Increase for stricter matches.')
